...
public class Integration {
	...
	if (this.supported && this.desktop.isSupported(Desktop.Action.APP_EVENT_HIDDEN)) {
	    this.desktop.addAppEventListener(listener);
	}
    }

    public void addAppReopenedListener(final AppReopenedListener listener) {
	if (this.supported && this.desktop.isSupported(Desktop.Action.APP_EVENT_REOPENED)) {
	    this.desktop.addAppEventListener(listener);
	}
    }

    public void addScreenSleepListener(final ScreenSleepListener listener) {
	if (this.supported && this.desktop.isSupported(Desktop.Action.APP_EVENT_SCREEN_SLEEP)) {
	    this.desktop.addAppEventListener(listener);
	}
    }

    public void addSystemSleepListener(final SystemSleepListener listener) {
	if (this.supported && this.desktop.isSupported(Desktop.Action.APP_EVENT_SYSTEM_SLEEP)) {
	    this.desktop.addAppEventListener(listener);
	}
    }

    public void addUserSessionListener(final UserSessionListener listener) {
	if (this.supported && this.desktop.isSupported(Desktop.Action.APP_EVENT_USER_SESSION)) {
	    this.desktop.addAppEventListener(listener);
	}
    }
	...
	if (this.supported && this.desktop.isSupported(Desktop.Action.BROWSE_FILE_DIR)) {
	...
	if (this.supported && this.desktop.isSupported(Desktop.Action.APP_SUDDEN_TERMINATION)) {
	...
	if (this.supported && this.desktop.isSupported(Desktop.Action.MOVE_TO_TRASH)) {
	...
	if (this.supported && this.desktop.isSupported(Desktop.Action.APP_HELP_VIEWER)) {
    ...
    public void removeAppForegroundListener(final AppForegroundListener listener) {
	if (this.supported && this.desktop.isSupported(Desktop.Action.APP_EVENT_FOREGROUND)) {
	...
	if (this.supported && this.desktop.isSupported(Desktop.Action.APP_PREFERENCES)) {
	...
	if (this.supported && this.desktop.isSupported(Desktop.Action.APP_PRINT_FILE)) {
	    this.desktop.setPrintFileHandler(printFileHandler);
	}
    }

    public void setQuitHandler(final QuitHandler quitHandler) {
	if (this.supported && this.desktop.isSupported(Desktop.Action.APP_QUIT_HANDLER)) {
	    this.desktop.setQuitHandler(quitHandler);
	}
    }

    public void setQuitStrategy(final QuitStrategy quitStrategy) {
	if (this.supported && this.desktop.isSupported(Desktop.Action.APP_QUIT_STRATEGY)) {
